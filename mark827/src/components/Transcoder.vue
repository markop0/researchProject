  ...
  >
    <div class="w-full max-w-md">
      <div v-if="!store.state.ffFileOk">
        <div class="pb-6">
          <div class="pb-2 font-bold">选择视频</div>
          <input
          ...
          <div class="pt-2 pb-2 text-sm text-gray-300">
            <span
              class="text-xs px-3 py-1 bg-161616/50 text-gray-300 rounded select-none mr-1"
              >原文件大小</span
            >
            {{
              store.state.ffFilesize
                ? sizeBytes(Number(store.state.ffFilesize))
                : "-"
            }}
            <span
              class="text-xs px-3 py-1 bg-161616/50 text-gray-300 rounded select-none ml-2 mr-1"
              >最小文件大小</span
            >
        ...
        <div class="pb-6">
          <div class="pb-2 font-bold">预设输入文件大小</div>
          <input
        ...
        <div class="pb-6">
          <div class="pb-2 font-bold">选择视频解析度</div>
          <div class="flex flex-wrap gap-x-2 gap-y-1">
            <div v-for="res in resScale" :key="res" :id="'res-' + String(res)">
              <button
                ...
                :class="
                  String(targetScale) == String(res)
                    ? 'bg-indigo-800/80 hover:bg-indigo-800/70'
          ...
          <div class="pt-2 pb-2 text-sm text-gray-300" v-if="targetScaleAuto">
            视频分辨率
          </div>
        </div>
        <div class="pb-2 text-right">
          ...
          >
            <span class="inline-block animate-pulse">⏳&ensp;压缩</span>
          </button>
          <button
      ...
      <div class="pt-6 pb-2" v-if="video">
        <div class="pb-2 font-bold">🎊 视频压缩结束 🎊</div>
        <video
          v-if="video"
          :src="video"
          ...
          controls
        />
        <div class="text-sm text-gray-300 pt-2">右键保存视频</div>
      </div>
    </div>
  </div>
</template>
...
watch(video, (newValue, oldValue) => {
  let a = store.state.timeRecord;
  let b = Date.now();
  let c = b - a;
  let h = Math.floor((c / 1000 / 60) << 0);
  let m = Math.floor((c / 1000) % 60);
  console.log(
    `压缩耗时： ${(h < 10 ? "0" : "") + h}:${(m < 10 ? "0" : "") + m}`
  );
});
...
const onFileChanged = async (event: Event) => {
  ...
  let targetScaleRatio =
    (Number(targetSize.value) * 1.5) /
    (Number(target.files[0].size) / 1024 / 1024);
  let targetScaleRecc = resScale.value.reduce(function (upper, lower) {
    return Math.min(lower, targetScaleRatio) != targetScaleRatio
      ? upper
      : lower;
  });
  targetScaleRecc != 1.0
    ? (targetScaleAuto.value = true)
    : (targetScaleAuto.value = false);
